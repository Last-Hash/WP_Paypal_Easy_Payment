...
add_shortcode('wp_paypal_payment_box_for_any_amount', 'wppep_buy_now_any_amt_handler');
...
add_shortcode('wp_paypal_payment_box', 'wppep_buy_now_button_shortcode');
...
<option value="HKD">HKD Hong Kong Dollar </option>
    ...
    if ($wp_pp_show_ref_box == '1') {
        $output .= '<div class="wppep_ref_title_label"><strong>'.esc_attr($wp_pp_ref_title).':</strong></div>';
        $output .= '<input type="hidden" name="on0" value="'.apply_filters('wp_pp_button_reference_name','Reference').'" />';
        $output .= '<div class="wppep_ref_value"><input type="text" name="os0" maxlength="60" value="'.apply_filters('wp_pp_button_reference_value','').'" class="wp_pp_button_reference" /></div>';
    }
